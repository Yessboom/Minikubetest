...
function up() {
  ...
  db.migrations.createIndex(
    { migration: 1 },
    { unique: true, name: "migration_unique" }
  );
  ...
  db.migrations.insertOne({
    ...
    description:
      "Initial schema with users, items, orders collections and sharding",
  ...
  print(
    "Sharding enabled for: users (hashed _id), items (userId, _id), orders (userId, orderDate)"
  );
